    ...
    def vend(self):
        if self.stock > 0:
            if self.balance > self.price:
                ...
                lack = self.price - self.balance
                return "You must add ${0} more funds.".format(lack)
        else:
...
def generate_paths(t, value):
    ...
    if t.label == value:
        yield [value]
    for b in t.branches:
        for i in generate_paths(b, value):
            yield [t.label] + i
...
def tree_min(t):
    if t.is_leaf():
        ...
        return min([t.label] + [tree_min(b) for b in t.branches])

def tree_max(t):
    if t.is_leaf():
        return t.label
    else:
        return max([t.label] + [tree_max(b) for b in t.branches]) 
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        brs = t.branches
        b_num = len(brs) <= 2
        all_b = all([is_bst(b) for b in brs])
        order = True
        if len(brs) == 2:
            order = tree_max(brs[0]) <= t.label < tree_min(brs[1])            
        return b_num and order and all_b
