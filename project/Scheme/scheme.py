...
def eval_all(expressions, env):
    ...
    temp = expressions
    while temp != nil:
        res = scheme_eval(temp.first, env)
        temp = temp.rest
    return res
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        t_formals, t_vals = formals, vals
        while t_formals != nil:
            child_frame.define(t_formals.first, t_vals.first)
            t_formals, t_vals = t_formals.rest, t_vals.rest
        return child_frame
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        # BEGIN PROBLEM 9
        temp = Pair(target.rest, expressions.rest)
        env.define(target.first, do_lambda_form(temp, env))
        return target.first
        # END PROBLEM 9
    else:
